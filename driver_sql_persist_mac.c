...
bool save_table_to_file(const Table *t, const char *path) {
    ...
    int32_t colc = t->column_count; uint64_t cnt = t->count;
    ...
    for (int i = 0; i < t->column_count; ++i) {
        if (t->columns[i].type == COL_INT) {
            if (t->count) { if (!write_exact(f, t->columns[i].data.int_data, t->count * sizeof(int))) goto fail; }
        } else if (t->columns[i].type == COL_TEXT) {
            if (t->count) { if (!write_exact(f, t->columns[i].data.text_data, t->count * MAX_TEXT_LEN)) goto fail; }
        }
    }
...
Table *load_table_from_file(const char *path) {
    ...
    char name[MAX_NAME_LEN]; int32_t colc; uint64_t cnt;
    ...
    char col_names[MAX_COLUMNS][MAX_NAME_LEN]; ColumnType types[MAX_COLUMNS];
    for (int i = 0; i < colc; ++i) {
        if (!read_exact(f, col_names[i], MAX_NAME_LEN)) goto fail;
        int32_t ctype; if (!read_exact(f, &ctype, sizeof(ctype))) goto fail; types[i] = (ColumnType)ctype;
    }
    const char *col_name_ptrs[MAX_COLUMNS]; for (int i = 0; i < colc; ++i) col_name_ptrs[i] = col_names[i];
    Table *t = create_table(name, colc, col_name_ptrs, types); if (!t) goto fail;
    if (!read_exact(f, t->deleted_bits, sizeof(t->deleted_bits))) { free_table(t); goto fail; }
    t->count = (cnt > t->capacity) ? t->capacity : (size_t)cnt;
    for (int i = 0; i < t->column_count; ++i) {
        if (t->columns[i].type == COL_INT) {
            if (t->count) { if (!read_exact(f, t->columns[i].data.int_data, t->count * sizeof(int))) { free_table(t); goto fail; } }
        } else if (t->columns[i].type == COL_TEXT) {
            if (t->count) { if (!read_exact(f, t->columns[i].data.text_data, t->count * MAX_TEXT_LEN)) { free_table(t); goto fail; } }
        }
    }
    fclose(f); return t;
fail:
